...
interface DroppableTabProps extends TabProps {
  ...
  onContextMenu: (event: React.MouseEvent | React.TouchEvent) => void // Allow TouchEvent as well
}
...
const DroppableTab: React.FC<DroppableTabProps> = ({ tab, onContextMenu, value, ...otherProps }) => {
  ...
  // useLongPress hook integration
  const longPressProps = useLongPress(
    (event) => {
      // The onContextMenu prop is already (event) => handleTabContextMenu(event, tab) from TabList
      onContextMenu(event)
    },
